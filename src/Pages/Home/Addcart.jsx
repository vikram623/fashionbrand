...
function Addcart() {
  ...
  let allcartproduct = () => {
    axios.get("https://fashionbrandbackend-b71e.vercel.app/allcart").then((res) => {
      if (res.data.status) {
  ...
  // console.log(data)
    ...
    let increagequantity = () => {
      ...
      axios.post("https://fashionbrandbackend-b71e.vercel.app/cartquantity",{qunatityitem,data})
    }
    ...
    let removercartitem = (item) => {
      axios.post("https://fashionbrandbackend-b71e.vercel.app/removecartitem", item).then((res) => {
        if (res.data.status) {
          Swal.fire({
