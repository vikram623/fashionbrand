...
function Wishlisting() {
    ...
    let wishitemget = () => {
        axios.get("https://fashionbrandbackend-pg8r.vercel.app/wishitems").then((res) => {
            if (res.data.status) {
...
let removewishlistitem=(item)=>{
    axios.post("https://fashionbrandbackend-pg8r.vercel.app/removewishlistitem",item).then((res)=>{
        if(res.data.status){
            Swal.fire({
