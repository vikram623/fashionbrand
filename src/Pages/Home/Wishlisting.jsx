...
function Wishlisting() {
    ...
    let wishitemget = () => {
        axios.get("https://fashionbrandbackend-b71e.vercel.app/wishitems").then((res) => {
            if (res.data.status) {
...
let removewishlistitem=(item)=>{
    axios.post("https://fashionbrandbackend-b71e.vercel.app/removewishlistitem",item).then((res)=>{
        if(res.data.status){
            Swal.fire({
